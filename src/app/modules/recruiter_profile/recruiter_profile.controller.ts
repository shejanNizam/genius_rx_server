...
const getProfileById = catchAsync(async (req: Request, res: Response) => {
  const profile = await RecruiterProfileServices.getProfileById(req.params.id as string);
  sendResponse(res, { statusCode: httpStatus.OK, success: true, message: "Profile retrieved successfully", data: profile });
});

const getAllProfiles = catchAsync(async (req: Request, res: Response) => {
  const result = await RecruiterProfileServices.getAllProfiles(req.query);
...
export const RecruiterProfileControllers = { upsertProfile, getMyProfile, getProfileById, getAllProfiles };
